...
def anchor_coordinate_transform(anchors, bbox_results, with_translation_transform=True, with_rotation_transform=True):
    ...
    for i in range(batch_size):
        ...
        if with_rotation_transform: 
            ...
            transformed_anchors = transformed_anchors.permute([0, 1, 2, 4, 3]) # 1, num_groups, num_modes, 12, 2 -> 1, num_groups, num_modes, 2, 12
            transformed_anchors = torch.matmul(rot_yaw, transformed_anchors).permute([0,1,2,4,3])# -> num_agents, num_groups, num_modes, 12, 2
...
def trajectory_coordinate_transform(trajectory, bbox_results, with_translation_transform=True, with_rotation_transform=True):
    ...
    for i in range(batch_size):
        ...
        if with_rotation_transform:
            ...
            transformed_trajectory = transformed_trajectory.permute([0,1,2,4,3]) # A, G, P, 12 ,2 -> # A, G, P, 2, 12
            transformed_trajectory = torch.matmul(rot_yaw, transformed_trajectory).permute([0,1,2,4,3])# -> A, G, P, 12, 2
        if with_translation_transform:
